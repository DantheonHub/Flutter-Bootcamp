...
- **Módulo 4 — Listas y formularios** *(en curso)*: `TextFormField` y validaciones, `Form`, selección (`Radio`/`Checkbox`/`Switch`, `Dropdown`), `ListView`, `GridView`, listas anidadas, CRUD en memoria con `setState`, y temas globales (colores por rol con `ColorScheme`, modo claro/oscuro, tipografía y temas de componentes), y diseño responsivo (widgets de distribución flexible, `MediaQuery`, `LayoutBuilder`, puntos de quiebre, patrones adaptativos y `flutter_screenutil_plus`), navegación (`Navigator`, rutas nombradas, `go_router`), internacionalización y una primera introducción a MVVM.
